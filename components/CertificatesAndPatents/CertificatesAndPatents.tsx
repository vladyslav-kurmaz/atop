...
const CertificatesAndPatents = ({
  ...
  const renderImage = (data: string[]) => {
    return data.map((item, i) => {
      return (
        <li className="certificateOrPatents__list-item" key={item} data-num={i}>
          <img
            src={item}
            alt={item}
            width={2000}
  ...
  return (
    <>
      {showSlider && initialSlide !== null && data ? (
        <SliderGalery
          data={data}
      ...
      <ul className="certificateOrPatents__list">{data && renderImage(data)}</ul>
    </>
  );
};
